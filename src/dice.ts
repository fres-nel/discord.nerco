...
export class Dice {
  ...
  private async d2(message: Message | PartialMessage) {
    let rand = Math.floor(Math.random() * 6 + Math.random() * 6 + 2);
  ...
  private async prodice(message: Message | PartialMessage) {
    ...
    sum = Math.floor(sum);
    await message.reply(" は " + before + "d" + after + "を振って **" + sum + "** を出した。");
    this.logger.log(LogType.Dice, message.author.username + " " + before + "d" + after + " : " + sum);
  }
}